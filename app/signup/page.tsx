...
export default function SignupPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-b from-poke-blue/5 via-background to-poke-yellow/5">
      ...
      {/* Main Content */}
      <div className="flex-1 flex items-center justify-center p-4 pt-12 pb-8">
        <div className="w-full max-w-sm space-y-5">
          ...
          <div className="text-center space-y-2">
            ...
            <p className="text-sm text-muted-foreground">
              Junte-se à comunidade de treinadores
            </p>
          </div>
                ...
                <div className="flex flex-wrap gap-x-3 gap-y-1 pt-1">
                  {passwordRequirements.map((req, i) => (
                    <div key={i} className="flex items-center gap-1">
                      <CheckCircle2 className={`h-3 w-3 ${req.met ? 'text-green-500' : 'text-muted-foreground/50'}`} />
                      <span className={`text-[10px] ${req.met ? 'text-green-500' : 'text-muted-foreground/70'}`}>
                        {req.text}
                      </span>
                    </div>
                  ))}
          ...
          <div className="relative">
            ...
            <div className="relative flex justify-center text-xs uppercase">
              <span className="bg-background px-3 text-muted-foreground">ou cadastre com</span>
            </div>
          </div>
          ...
          <p className="text-center text-sm text-muted-foreground">
            Já tem uma conta?{' '}
            <Link href="/login" className="text-poke-blue hover:underline font-semibold">
              Fazer login
            </Link>
          </p>
